...
@interface RxWebViewController ()<UIWebViewDelegate,UINavigationControllerDelegate,UINavigationBarDelegate,NJKWebViewProgressDelegate,WKNavigationDelegate,WKUIDelegate>
...
- (void)viewDidLoad{
    ...
    SmartJSWebView *webView = [[SmartJSWebView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, ScreenHeight)];
    webView.proxyDelegate.realDelegate = self;
...
-(void)loadPage:(NSURL *)pageURL{
    [self.webView addJavascriptInterfaces:[MyJSInterface sharedInstance] WithName:@"BDInterface"];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    self.webView.frame = CGRectMake(0, 0, ScreenWidth, keyboardRect.origin.y);
    // 你的操作，如键盘出现，控制视图上移等
}
...
-(UIBarButtonItem*)customBackBarItem{
    if (!_customBackBarItem) {
        UIImage* backItemImage = [[UIImage imageNamed:@"navbar_back_withlighttext"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        UIImage* backItemHlImage = [[UIImage imageNamed:@"navbar_back_withlighttext"] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
